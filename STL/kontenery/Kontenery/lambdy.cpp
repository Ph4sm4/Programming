...
int main() {	
	...
	int wciag_mnie = 1;
	[wciag_mnie] {	
		cout << "Wciągnąłem wartość: " << wciag_mnie << endl; // tak wciągnięta wartość nie może być modyfikowana
	}();
	cout << wciag_mnie << endl;
	[&wciag_mnie] {	 // a tu wciągam przez referencję, modyfikacja wciągniętej możliwa
		++wciag_mnie;
		cout << "Wciągnąłem i zmieniłem wartość: " << wciag_mnie << endl;
	}();
	cout << wciag_mnie << endl; // lambda zmieniła wartość

	// lambda może mieć argumenty i coś zwracać
	auto lambda2 = [wciag_mnie](int a, const double& b) {
		return (wciag_mnie + a) * b;
	};
	...
	auto lambda3 = [wciag_mnie](int a, const double& b) -> long int {
		return (long int)((wciag_mnie + a) * b); // 
	};
	...
	int var2 = 2;
	[=] {
		cout << "Wciągam wszystko zewnętrzne do lambdy = " << wciag_mnie << " " << var1 << " " << var2 << endl; // zmienne jednak jak var1 itp. nie mogą być zmieniane
	}();
